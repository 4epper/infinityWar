...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        HashMap<String, String> edg = new HashMap<>();
        
        int v = StdIn.readInt();
        for (int i = 0; i < v; i++) {
            vertex.add(StdIn.readString());
        }

        int e = StdIn.readInt();
        for (int i = 0; i < e; i++) {
            edg.put(StdIn.readString(), StdIn.readString());
        }

        String mindStone = "";
        for (String vert : vertex) {
            if (!edg.containsKey(vert)) {
                mindStone = vert;
                break;
            }
        }

        for (String key : edg.keySet()) {
            if (edg.get(key).equals(mindStone)) {
                StdOut.println(key);
            }
        }
    }
